...
### 6-1. コードレビュー呼び出し（並列実行）

🚨 **最重要**: 以下の2つのTaskを**必ず同一メッセージ内で並列実行**してください。

変更ファイルを検出して、code-reviewerエージェントとCodex MCPレビューを並列実行します：
...
**詳細**: 各問題の詳細説明、修正提案、コード例、参照ドキュメント、総評、良い点もすべて表示
...
**P0/P1/Critical項目なし**: Step 7へ進む ✅

**P0/P1/Critical項目あり**: 以下のフローで対応

1. **修正対象を判断**
   - **P0 & Critical**: すべて修正必須
   - **P1 & Important**: 実装工数と影響を考慮して判断
...
3. **修正後の判定**
   - 全P0/Critical項目を修正完了: Step 7へ進む
   - 未修正のP0/P1/Critical項目が残る: エラーで中断

**エラーメッセージ**（未修正のP0/P1/Critical項目が残る場合）:
```
...
- ✅ コード簡素化提案完了（公式プラグイン）
- ✅ ユニットテスト成功 [or スキップ]
- ✅ セルフレビュー完了（code-reviewer + Codex MCP）
  - code-reviewer: P0/P1なし
  - Codex MCP: Critical/Importantなし
